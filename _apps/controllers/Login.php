...
class Login extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['page_title'] = "Login - ".$title->option_value;
		...
		$template = '/frontend/content/login';
	...
	public function proses(){
		...
		//cek email database
		if($this->Usersmodel->CekUserStatus($user_email)->result())
		{

			//cek login
			$row = $this->Usersmodel->cekLogin($user_email,$_password)->row();
			if($row)
			{
				...
				$this->Usersmodel->update_cookie($update_key, $row->user_id);
	...
	public function forgot()
	{
		...
		$title = $this->Mainmodel->GetOptions("web_title");
		$data['page_title'] = "Forgot Password - ".$title->option_value;
		...
		$template = '/frontend/content/forgot';
	...
	public function forgot_proses()
	{
		...
		else
		{
			...
			//cek akun aktif
			if($this->Usersmodel->CekUserStatus($email)->row()->user_status != "1")
			{

				$array_msg = array(
								'type' => 'danger',
								'message' => 'Akun anda tidak dalam keadaan aktif, Silahkan Ulangi Lagi.'
							);
				$this->session->set_flashdata('message_flashdata', $array_msg);
				redirect('login/forgot');
			...
			$resend = $this->Usersmodel->DetailEmail($email)->row();

			if($resend)
			{

				//send email
				$url = base_url()."reset/reset/".$resend->username."/".$resend->user_email."/".$resend->user_newsletter."/".$resend->activation_code."/".$resend->user_status."/".$resend->cookie."";
				$this->_send_email_forgot($url,$this->input->post('email'),$resend->username);
	...
	//==================== save mass email =================== //
	private function _send_email_forgot($url,$email,$username){
		...
		$mail->Host = 'mail.laxo.net.id'; // host sesuaikan dengan hosting mail anda
		$mail->Port       = 587;  // post gunakan port 25
		//$mail->Port       = 465;  // post gunakan port 25
		$mail->Username   = "noreply@laxo.net.id"; // username email akun
		$mail->Password   = "noreply123!!";        // password akun
		
		$mail->SetFrom('noreply@laxo.net.id', $this->Mainmodel->GetOptions("web_title")->option_value);
		
		$mail->Subject    = "Password Reset";
		//$html = "<div>Link : ".$url."</div>";
		$html = $this->_email_body_forgot($url,$email,$username);
		$mail->MsgHTML($html);
		//*/
		...
		if(!$mail->Send()) {
			...
			$array_msg = array(
							'type' => 'success',
							'message' => 'Kami telah mengirimi Anda tautan reset kata sandi. Silakan cek email anda.'
						);
			$this->session->set_flashdata('message_flashdata', $array_msg);
			redirect('login/forgot');
	...
	private function _email_body_forgot($url,$email,$username)
	{
		return
		'
						...
						<table>
							<tr>
								<td>
									...
									<!-- Callout Panel -->
									<p class="callout">
										Anda menerima pemberitahuan dari '.$this->Mainmodel->GetOptions("web_title")->option_value.'. Karena anda melakukan reset password di situs kami, silahkan akses link yang kami kirim dan reset password anda.
									</p><!-- /Callout Panel -->
	...
	public function _newsletter_email()
	{
		return
		'
			<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
