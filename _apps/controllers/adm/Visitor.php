...
class Visitor extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('adm/Visitormodel');
    }

    public function index() {
        $site_statics_today = $this->Visitormodel->get_site_data_for_today();
        $site_statics_last_week = $this->Visitormodel->get_site_data_for_last_week();
        ...
        $data['chart_data_today'] = $this->Visitormodel->get_chart_data_for_today();
        $data['chart_data_curr_month'] = $this->Visitormodel->get_chart_data_for_month_year();
        $this->load->view('backend/content/visitor', $data);
        //print_r($data);
    }
    ...
    function get_chart_data() {
        if (isset($_POST)) {
            if (isset($_POST['month']) && strlen($_POST['month']) && isset($_POST['year']) && strlen($_POST['year'])) {
                $month = $_POST['month'];
                $year = $_POST['year'];
                $data = $this->Visitormodel->get_chart_data_for_month_year($month, $year);
                if ($data !== NULL) {
                ...
                $month = $_POST['month'];
                $data = $this->Visitormodel->get_chart_data_for_month_year($month);
                if ($data !== NULL) {
                    foreach ($data as $value) {
                        echo $value->day . "t" . $value->visits . "n";
                    }
                ...
                $year = $_POST['year'];
                $data = $this->Visitormodel->get_chart_data_for_month_year(0, $year);
                if ($data !== NULL) {
                    foreach ($data as $value) {
                        echo $value->day . "t" . $value->visits . "n";
                    }
                ...
                $data = $this->Visitormodel->get_chart_data_for_month_year();
                if ($data !== NULL) {
                    foreach ($data as $value) {
                        echo $value->day . "t" . $value->visits . "n";
                    }
